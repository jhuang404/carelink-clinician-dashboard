...
export async function GET() {
  ...
  const hasFirebase = !!process.env.FIREBASE_ADMIN_PROJECT_ID;
  const allEnvKeys = Object.keys(process.env).filter(k => k.includes("RESEND") || k.includes("FIREBASE_ADMIN")).sort();
  return NextResponse.json({ hasKey, keyPrefix, hasFirebase, envKeysMatching: allEnvKeys, nodeEnv: process.env.NODE_ENV });
}
